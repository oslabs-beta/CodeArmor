...
import { applyIamWildcardRule } from '../staticAnalysis/iamRule'; // brings rule for Iam permissions
...
export function analyzeCode(code: string, document: vscode.TextDocument) {
  console.log('🔍 Code received for analysis');
  ...
  if (hasLambdaHandler) {
    console.log('✅ Lambda handler detected — running deep checks');

    traverse(ast, {
      ObjectExpression(path) {
        const message = applyIamWildcardRule(path.node);
        if (message) {
          console.log('🚨 IAM wildcard violation:', message);
      ...
      StringLiteral(path) {
        ...
        if (message) {
          console.log('🚨 AWS Secret detected:', message);
    ...
    console.log('no lambda handler found');
  }
  console.log('diagnostics collected', diagnostics.length);
