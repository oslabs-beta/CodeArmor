...
export function sqlInjectionRule(
  ...
  const sqlInjectionPatterns = [
    ...
    {
      pattern:
        /(SELECT|INSERT|UPDATE|DELETE|FROM|WHERE|VALUES|SET)\s+[^;]{0,100}?\+\s*[a-zA-Z_$][\w$]*/gi,
    ...
    {
      pattern:
        /(SELECT|INSERT|UPDATE|DELETE|FROM|WHERE|VALUES|SET)\s+[^`]{0,50}?\$\{[^}]+\}[^`]{0,50}?/gi,
      message:
        'Variable interpolation in SQL template literal can lead to SQL injection',
      severity: vscode.DiagnosticSeverity.Error,
    },
    // Variable inside single quotes (SQL string context)
    {
      ...
      message:
        'Variable concatenation inside SQL quotes can lead to SQL injection',
      severity: vscode.DiagnosticSeverity.Error,
    },
    // Query methods with concatenated strings (more specific)
    {
      pattern:
        /\.(query|execute|run)\(\s*[^?][^,]{0,50}?\+\s*[a-zA-Z_$][\w$]*[^)]*\)/gi,
      message:
        'Query method with string concatenation can lead to SQL injection',
      severity: vscode.DiagnosticSeverity.Error,
    },
  ];
  ...
  for (const { pattern, message, severity } of sqlInjectionPatterns) {
    ...
    while ((match = pattern.exec(code)) !== null) {
      ...
      const isSafe = safePatterns.some(
        (safePattern) =>
      ...
      if (!isSafe) {
        try {
          ...
          const diagnostic = new vscode.Diagnostic(
            range,
            `[SECURITY] ${message} - Found: "${matchText.substring(0, 40)}${
              matchText.length > 40 ? '...' : ''
            }"`,
            severity
          );
